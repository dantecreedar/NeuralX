class ChatNeuron {
    private logic: Record<string, Record<string, string[]>> = {
        "hola": {
    ...
    public async getResponse(message: string, mood: string): Promise<string> {
    ...
    private fetchUserData = async (): Promise<any[]> => {
    ...
    private findSimilarWords = (message: string): string[] => {
        const threshold = 0.5;
...
function sendMessage(): void {
    ...
    const previousResponse: HTMLElement = chat.querySelector('.typing-effect') as HTMLElement;
    if (previousResponse) {
        previousResponse.remove();
    }

    // Mostrar el efecto de "cargando"
    const loadingMessage: HTMLDivElement = document.createElement('div');
    ...
    chatNeuron.getResponse(userMessage, currentMood)
        .then(botResponse => {
            ...
            const responseContainer: HTMLDivElement = document.createElement('div');
            responseContainer.classList.add('typing-effect');
            chat.appendChild(responseContainer);

            const characters: string[] = botResponse.split('');
